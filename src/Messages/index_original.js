...
class Messages extends Component{
	...
	handleSubmit = async(e) => {
		...
		try{
			...
			const parsedResponse = await response.json();
			this.setState({
				message: [...this.state.message, parsedResponse.message]
			})
			this.sendSocket(parsedResponse.message);
			this.setState({text:""});
		}catch(err){
	...
	render(){
		// console.log("updated state from messages");
				...
				<form onSubmit={this.handleSubmit} className="messageForm">
					<div className="row">
						<div className="col-lg-10">
							<input id="message_input" className="form-control no-border" type="text" value={this.state.text} name="text" placeholder="Write a message ..." onChange={this.handleChange}/>
						</div>
